...
$query1="INSERT into grade1 (name,enroll,C,DE,BO,EM,CS,C_pr,DE_pr,CS_pr,total)values(?,?,?,?,?,?,?,?,?,?,?)";
$stmt1=$con->prepare($query1);
$stmt1->bind_param("sssssssssss",$name,$enroll,$g1,$g2,$g3,$g4,$g5,$g6,$g7,$g8,$tg);
$query = "INSERT into firstsem(name,dob,enroll,sem,C,DE,BO,EM,CS,C_pr,DE_pr,CS_pr,total,rem)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $con->prepare($query);
$stmt->bind_param("sssissssssssii",$name,$dob,$enroll,$sem,$first,$second,$third,$fourth,$fifth,$sixth,$seventh,$eighth,$total,$rem);
$stmt->execute();
$stmt1->execute();
$stmt->close();
$stmt1->close();
...
<input type="submit" id="btn" name="add1" value ="Continue">
</form>
</div>
</html>
<?php
